...
export class SyncBuffer implements utl.Resource<DataBuffer> {
    ...
    set<T>(element: typ.Element<T>, value: T) {
    ...
    static instances<D extends DataBufferDescriptors>(device: Device, descriptors: D, labelPrefix?: string): SyncBuffers<D> {
        return utl.replaceValues(descriptors, (descriptor, key) => 
            SyncBuffer.instance(device, utl.withLabel(descriptor, labelPrefix, key))
        )
    }
...
export class DataBuffer extends AbstractBuffer implements utl.Resource<DataBuffer>, SourceBuffer, DestinationBuffer {
    ...
    static instances<D extends DataBufferDescriptors>(device: Device, descriptors: D, labelPrefix?: string): DataBuffers<D> {
        return utl.replaceValues(descriptors, (descriptor, key) => 
            DataBuffer.instance(device, utl.withLabel(descriptor, labelPrefix, key))
        )
    }
...
export class ReadBuffer extends AbstractBuffer implements DestinationBuffer {
    ...
    private mapper = new BufferMapper(this.wrapped, GPUMapMode.READ)
    ...
    static instances<D extends ReadBufferDescriptors>(device: Device, descriptors: D, labelPrefix?: string): ReadBuffers<D> {
        return utl.replaceValues(descriptors, (descriptor, key) => 
            ReadBuffer.instance(device, descriptor, utl.label(labelPrefix, key))
        )
    }
...
export class WriteBuffer extends AbstractBuffer implements SourceBuffer {
    ...
    private mapper: BufferMapper = new BufferMapper(this.wrapped, GPUMapMode.WRITE)
    ...
    static instances<D extends WriteBufferDescriptors>(device: Device, descriptors: D, labelPrefix?: string): WriteBuffers<D> {
        return utl.replaceValues(descriptors, (descriptor, key) => 
            WriteBuffer.instance(device, descriptor, utl.label(labelPrefix, key))
        )
    }
...
function doCopy(
    ...
    dstOffset: number,
    size: number = Math.min(srcBuffer.size - srcOffset, dstBuffer.size - dstOffset), 
) {
...
class BufferMapper {
    ...
    private mappedPromise: Promise<void> = Promise.resolve()
    private unmappedPromise: Promise<void> = Promise.resolve()
    private unmappedResolve: (value?: any) => void = () => {}

    constructor(private buffer: GPUBuffer, private mode: GPUMapModeFlags) {
    }

    async request(offset: number, size: number): Promise<Uint8Array> {
    ...
    private async doRequest(offset: number, size: number) {
